...
const MobileBottomDock = () => {
  ...
  return (
    <nav className="sm:flex md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm px-4">
      <div className="flex items-center justify-around gap-1 px-4 py-3 bg-background border border-border rounded-full shadow-2xl">
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? "text-primary bg-accent" : "text-foreground/70 hover:text-foreground hover:bg-accent/50"
              )}
            >
              {item.label}
            </Link>
